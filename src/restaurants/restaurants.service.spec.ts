...
describe("RestaurantService", () => {
  ...
  describe("create", () => {
    ...
    it("should create a new restaurant", async () => {
      jest.spyOn(model, "create").mockResolvedValue(newRestaurant as any);
  ...
  describe("findById", () => {
    ...
    it("should throw wrong moongose id error", async () => {
      await expect(service.findById("wrong_id")).rejects.toThrow(BadRequestException);
    });
  ...
  describe("uploadImages", () => {
    ...
    it("should upload restaurant images local", async () => {
      const mockImages = [
        "image1.jpeg"
      ];
      ...
      jest.spyOn(APIFeatures, "uploadLocal").mockResolvedValueOnce(mockImages);
      jest.spyOn(model, "findByIdAndUpdate").mockResolvedValueOnce(updatedRestaurant as any);
      ...
      const result = await service.uploadImages(mockRestaurant._id, files);
      expect(APIFeatures.uploadLocal).toHaveBeenCalled();
      expect(result).toEqual(updatedRestaurant);
    });
  ...
  describe("deleteImages", () => {
    ...
    it("should delete restaurant images local", async () => {
      const mockImages = [
        "image1.jpeg"
      ];

      jest.spyOn(APIFeatures, "deleteImagesLocal").mockResolvedValueOnce(true);

      const result = await service.deleteImages(mockImages);
      expect(APIFeatures.deleteImagesLocal).toHaveBeenCalled();
      expect(result).toBe(true);
    });
